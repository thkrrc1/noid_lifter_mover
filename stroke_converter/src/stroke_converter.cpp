...
void seed_converter::NoidLifterMover::makeTables() {
    upper_csv_dir = ament_index_cpp::get_package_share_directory("noid_lifter_mover") + "/csv/typeG_upperbody";
    lifter_csv_dir = ament_index_cpp::get_package_share_directory("noid_lifter_mover") + "/csv/typeG_lifter";
...
int16_t seed_converter::NoidLifterMover::calcStroke2(double angle,double scale,int dir,double offset, const std::vector<seed_converter::StrokeMap> &_table){
    static const double rad2Deg = 180.0 / M_PI;
    if (std::isnan(angle)) {
        return 0x7FFF;
    }
...
void seed_converter::NoidLifterMover::calcStroke3(double angle1,double angle2,double scale,int dir1,int dir2, const std::vector<seed_converter::StrokeMap> &_table1, const std::vector<seed_converter::StrokeMap> &_table2,bool is_pitch,int16_t& out1,int16_t& out2){
    static const double rad2Deg = 180.0 / M_PI;
...
void seed_converter::NoidLifterMover::Angle2Stroke(std::vector<int16_t> &_strokes, const std::vector<double> &_angles) {
...
int16_t seed_converter::NoidLifterMover::calcWheelAngDeg(int idx, int16_t raw_ang) {
...
void seed_converter::NoidLifterMover::Stroke2Angle(std::vector<double> &_angles, const std::vector<int16_t> &_strokes) {
...
void seed_converter::NoidLifterMover::setJointNames(const std::vector<std::string> &names) {
...
void seed_converter::NoidLifterMover::calcActuatorVel(std::vector<int16_t> &actuator_vel, const std::vector<double> &joint_vel) {
...
void seed_converter::NoidLifterMover::calcJointVel(std::vector<double> &joint_vel,const std::vector<int16_t> &actuator_vel){
...
PLUGINLIB_EXPORT_CLASS(seed_converter::NoidLifterMover, seed_converter::StrokeConverter)
